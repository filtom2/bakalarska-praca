...
class DeformableHeadAttention(nn.Module):
    ...
    def __init__(self,last_height,last_width, C, M=8, K=4, L = 1, dropout=0.1, return_attentions = False,
                 use_zoom_in_bounding=True, use_zero_bias_init=True, scale_factors_list=None):
        """
        Args:
            ...
            - param use_zoom_in_bounding: boolean, whether to apply tanh and scale factors
            - param use_zero_bias_init: boolean, whether to initialize zero bias for delta_proj
            - param scale_factors_list: list of scale factors per level (default: [2.0, 4.0, 8.0, 16.0])
        """
        ...
        self.use_zoom_in_bounding = use_zoom_in_bounding
        self.use_zero_bias_init = use_zero_bias_init
        if scale_factors_list is None:
            self.scale_factors_list = [2.0, 4.0, 8.0, 16.0][:self.L]  # Ensure it matches L
        else:
            self.scale_factors_list = scale_factors_list[:self.L]
    ...
    def forward(self,z_q,Xs,p_q ,query_mask = None,x_masks = None):
        ...
        if self.use_zoom_in_bounding:
            # ZOOM-IN DEFORMABLE ATTNETION 
            # Tanh for range between -1 and 1
            bounded_deltas = torch.tanh(deltas)
    
            # Scale limit (pixels)
            scale_factors = torch.tensor(self.scale_factors_list, device=deltas.device)
    
            # Reshape to broadcast across (B, H, W, M, L, K, 2)
            scale_factors = scale_factors.view(1, 1, 1, 1, self.L, 1, 1)
            # Apply the Zhoom-in
            deltas = bounded_deltas * scale_factors
    ...
    def init_parameters(self):
        ...
        if self.use_zero_bias_init:
            # Important: Initialize bias to 0.0 so that tanh(0) = 0, preventing saturation
            # and starting the attention directly at the reference point before expanding.
            torch.nn.init.constant_(self.delta_proj.bias, 0.0)
        else:
            # Standard Deformable DETR initialization
            thetas = torch.arange(self.M, dtype=torch.float32) * (2.0 * math.pi / self.M)
            grid_init = torch.stack([thetas.cos(), thetas.sin()], -1)
            grid_init = (grid_init / grid_init.abs().max(-1, keepdim=True)[0]).view(self.M, 1, 1, 2).repeat(1, self.L, self.K, 1)
            for i in range(self.K):
                grid_init[:, :, i, :] *= i + 1
            self.delta_proj.bias.data = grid_init.view(-1)
