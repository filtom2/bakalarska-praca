...
def make_mitos_transforms(image_set, args=None):
    """
    ...
    if image_set == 'train':
        ...
        if not getattr(args, 'no_color_aug', False):
            transforms_list.append(T.RandomColorJitter(brightness=0.2, contrast=0.2, saturation=0.1, hue=0.05))
            if not getattr(args, 'no_stain_aug', False):
                transforms_list.append(T.RandomStainAugmentation(sigma_l=0.05, sigma_a=0.08, sigma_b=0.08, p=0.5))
...
def build(image_set, args):
    ...
    dataset = MitosDetection(
        str(img_folder), 
        str(ann_file), 
        transforms=make_mitos_transforms(image_set, args), 
        return_masks=False,
